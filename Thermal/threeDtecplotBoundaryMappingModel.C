  ...
  if(foundInList(fieldName,mappingFieldsName_ )) {
    ...
    if (mesh_.objectRegistry::foundObject<volScalarField>(mappingFieldsName_[fieldI])) {
      ...
      forAll(field_.boundaryField()[patchID], faceI) {
        ...
        //fieldsInterp_.append(mappingFileDataI_[mappingFieldsColumn_[fieldI]][indexData]);        
        if(debug_) {
        ...
        //fieldsInterp_.append(mappingFileDataIPlus1_[mappingFieldsColumn_[fieldI]][indexData]);        
...
Foam::scalar Foam::threeDtecplotBoundaryMappingModel::interpolateNearestFaceCOG(label patchID, label faceI, label fieldI, List<scalarList>& mappingFileData)
{
  if(debug_) {
    Info << "--- start ---  Foam::BoundaryMapping::interpolateNearestFaceCOG(label patchID, label faceI, label fieldI, List<scalarList>& mappingFileData)" << endl;
  }
  ...
  double xp = mesh_.C().boundaryField()[patchID][faceI][0];
  double yp = mesh_.C().boundaryField()[patchID][faceI][1];
  double zp = mesh_.C().boundaryField()[patchID][faceI][2];    
  ...
  if (indexData >= 0) {
    ...
    if(debug_) {
      Info << "--- (xp=" << xp << ",yp=" << yp << ",zp=" << zp << ") => " << mappingFields_[fieldI].first() << "(" << xFileData_[indexData] << "," << yFileData_[indexData]  << "," << zFileData_[indexData] << ") = " <<  mappingFileData[mappingFieldsColumn_[fieldI]][indexData] <<
           " ---  Foam::BoundaryMapping::interpolateNearestFaceCOG(label patchID, label faceI, label fieldI, List<scalarList>& mappingFileData)" << endl;
    }
  }

  if(debug_) {
    Info << "--- end ---  Foam::BoundaryMapping::interpolateNearestFaceCOG(label patchID, label faceI, label fieldI, List<scalarList>& mappingFileData)" << endl;
  }
  return value_;  
}
