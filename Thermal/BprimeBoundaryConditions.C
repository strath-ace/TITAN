...
nameBprimeFile_(dict_.lookupOrDefault<fileName>("BprimeFile",fileName(word::null).expand()))
{
  ...
  //scalarFields_.insert("ce(0)",energyModel_.createVolField<scalar>("ce(0)",dimensionedScalar("0", dimless, scalar(0))));
  //scalarFields_.insert("ce(1)",energyModel_.createVolField<scalar>("ce(1)",dimensionedScalar("0", dimless, scalar(0))));
  //scalarFields_.insert("ce(2)",energyModel_.createVolField<scalar>("ce(2)",dimensionedScalar("0", dimless, scalar(0))));
  //scalarFields_.insert("cwall(0)",energyModel_.createVolField<scalar>("cwall(0)",dimensionedScalar("0", dimless, scalar(0))));
  //scalarFields_.insert("cwall(1)",energyModel_.createVolField<scalar>("cwall(1)",dimensionedScalar("0", dimless, scalar(0))));
  //scalarFields_.insert("cwall(2)",energyModel_.createVolField<scalar>("cwall(2)",dimensionedScalar("0", dimless, scalar(0))));
  scalarFields_.insert("molten",energyModel_.createVolField<scalar>("molten",dimensionedScalar("0", dimless, scalar(0))));
...
void Foam::BprimeBoundaryConditions::initialize()
{
  ...
  scalar init_mass = constantPropertiesDictionary.lookupOrDefault<scalar>("mass", 0.0);
  scalar init_density = constantPropertiesDictionary.lookupOrDefault<scalar>("density", 0.0);
  mass = init_mass;
...
//  // Access the boundary patch with currentPatchID_
//  const fvPatch& currentPatch = mesh_.boundary()[currentPatchID_];
//
//  // Get the number of faces in the specified boundary patch
//  label nBoundaryFacets = currentPatch.size();
//
//  molten = new int[nBoundaryFacets];
//
//  for (int i = 0; i < nBoundaryFacets; i++) {
//    molten[i] = 0;
//
//    Info << "\n\n\n\n molten["<<i<<"]: " << molten[i] << "\n\n\n" << endl;
//  }
...
void Foam::BprimeBoundaryConditions::initBprimeMixture()
{
  ...
  if (nameBprimeFile_!=word::null && !writeBprimeTable_) {
    ...
    Info << "ne_Bprime:" << ne_Bprime << endl;

    for (int i = 0; i < ne_Bprime; i++) {
      Info << "element name:" <<  mixBprime_.elementName(i) << endl;
    }
    raise(SIGTERM);

    //Info << "ns_Bprime:" << ns_Bprime << endl;
    Yke_ref = new double[ne_Bprime];
    Ykg_ref= new double[ne_Bprime];

//    //Info << "initBprimeMixture() 1" << endl;
//
...
//    for (int i = 0; i < ne_Bprime; i++) {
//      //Info << "mixBprime_.elementName(i): " << mixBprime_.elementName(i)  << endl;
//      Yke_ref[i] =
...
//    }
    ...
    bool atLeastOneElement = false;
    for (int i = 0; i < ne_Bprime; i++) {
      word name_ = moleFractionGasInMaterial_[i].first();
      scalar value_ =  moleFractionGasInMaterial_[i].second();
      Ykg_ref[i] = value_;
      if (Ykg_ref[i]!=0) {
        atLeastOneElement = true;
      }
...
//    }
  ...
  //raise(SIGTERM);
  Info << "if (writeBprimeTable_)" << endl;
  if (writeBprimeTable_) {
    Info << "inside if (writeBprimeTable_)" << endl;
    writeBprimeTable();
...
void Foam::BprimeBoundaryConditions::updateBoundaryMapping()
{
  ...
  //Info << "updateBoundaryMapping()" << endl;
  ...
  //forAll(rhoeUeCH.boundaryFieldRef()[currentPatchID_], faceI) {
...
void Foam::BprimeBoundaryConditions::updateMotionBC()
{
  ...
  volScalarField& mDotMelt_=scalarFields_["mDotMelt"];
  ...
  if(mesh_.objectRegistry::foundObject<volVectorField>("cellMotionU")) {
    ...
    forAll(cellMotionU_.boundaryFieldRef()[currentPatchID_], faceI) {
      ...
      const scalar& mDotMelt_BF = mDotMelt_.boundaryField()[currentPatchID_][faceI];
      ...
      cellMotionU_BF=
          (BprimeCw_BF* rhoeUeCH_BF * blowingCorrection_BF * (1 + failureFraction)
           + erosionRate + mDotMelt_BF)
          * 1 / rho_s_bf * normal_;
      if (!recessionFlag) {
        cellMotionU_BF = vector(0,0,0);
      }

      //Info << "mDotMelt_BF: " << mDotMelt_BF << endl;
      //Info << "cellMotionU_BF: " << cellMotionU_BF << endl;
    }
  }
...
void Foam::BprimeBoundaryConditions::updateTemperatureBC()
{
  ...
  volScalarField& mDotMelt_=scalarFields_["mDotMelt"];
  volScalarField& mDotVapor_=scalarFields_["mDotVapor"];
  ...
  volScalarField& rho_s_=scalarFields_["rho_s"];
  volScalarField& molten_=scalarFields_["molten"];

  scalar Tmelt = constantPropertiesDictionary.lookupOrDefault<scalar>("Tmelt", 0.0);
  scalar Hfusion = constantPropertiesDictionary.lookupOrDefault<scalar>("Hfusion", 0.0);
  scalar Tboil = constantPropertiesDictionary.lookupOrDefault<scalar>("Tboil", 0.0);
  scalar Hboil = constantPropertiesDictionary.lookupOrDefault<scalar>("Hboil", 0.0);
  scalar fstrip = constantPropertiesDictionary.lookupOrDefault<scalar>("fstrip", 0.0);
  scalar mass = constantPropertiesDictionary.lookupOrDefault<scalar>("mass", 0.0);


  double time_step = mesh_.time().deltaT().value();

  double curr_time = mesh_.time().value();

  double end_time = mesh_.time().endTime().value();

  //Info << "rhoeUeCH: " << rhoeUeCH << endl;
  //Info << "h_w: " << h_w << endl;
  //Info << "curr_time: " << curr_time << endl;
...
//  const scalar& rhoeUeCH_test = rhoeUeCH.boundaryFieldRef()[currentPatchID_]
//
//  forAll(T_.boundaryFieldRef()[currentPatchID_], faceI) {
//    Info << "rhoeUeCH_test[faceI]: " << rhoeUeCH_test[faceI] << endl;
//
//  }

  //scalar& rhoeUeCH_test2 = rhoeUeCH.boundaryFieldRef()[currentPatchID_]

  //forAll(T_.boundaryFieldRef()[currentPatchID_], faceI) {

    //const scalar& rhoeUeCH_test = rhoeUeCH.boundaryFieldRef()[currentPatchID_][faceI];

    //Info << "rhoeUeCH_test: " << rhoeUeCH_test << endl;
...
//    Info << "\nfaceI: " << faceI << endl;
//
//    scalar& rhoeUeCH_test2 = rhoeUeCH.boundaryFieldRef()[currentPatchID_][faceI];
//
//    Info << "rhoeUeCH_test2: " << rhoeUeCH_test2 << endl;
//
//  }
  //raise(SIGTERM);

  scalar localMassChange = 0.0;
  //double fStrip = 0.5;
  ...
  forAll(T_.boundaryFieldRef()[currentPatchID_], faceI) {
    ...
    scalar& mDotMelt_BF = mDotMelt_.boundaryFieldRef()[currentPatchID_][faceI];
    scalar& mDotVapor_BF = mDotVapor_.boundaryFieldRef()[currentPatchID_][faceI];
    //scalar& rhoeUeCH_BF = rhoeUeCH.boundaryFieldRef()[currentPatchID_][faceI];
    scalar& molten_BF = molten_.boundaryFieldRef()[currentPatchID_][faceI];
...
//    }
    ...
    if  (T_BF > Tmelt)  {
      ...
      //Info << " qCond_BF: " << qCond_BF << endl;
      //Info << " qConv_BF: " << qConv_BF << endl;
      //Info << " Qmelt: " << Qmelt << endl;
      //Info << " mDotMelt_BF: " << mDotMelt_BF << endl;
  ...
  // Reference the 'rho_s' field via energyModel (or the appropriate model managing rho_s)
  volScalarField& rho_s = energyModel_.refVolField<scalar>("rho_s");


  // Modify the internal field values directly
  forAll(rho_s, celli)
  {
      rho_s[celli] = density;  // Set all internal field values to the updated density
      //Info << "rho_s[celli]:" << rho_s[celli] << endl;
  }

  rho_s_.boundaryFieldRef() = density;

  forAll(rho_s_.boundaryFieldRef()[currentPatchID_], faceI) {
    scalar& rho_s_BF = rho_s_.boundaryFieldRef()[currentPatchID_][faceI];
    //Info << "rho_s_BF:" << rho_s_BF << endl;
  }

  // Ensure the changes are propagated throughout the simulation by updating boundary conditions
  rho_s.correctBoundaryConditions();
  ...
  if (massFile){
    ...
    const word fileName = "massFile";  // File name
    const word directory = "uniform"; // Directory for output, can be "uniform" or "constant"

    // Create the IOdictionary for managing the file data
    IOdictionary massDict
    (
        IOobject
        (
            fileName,
            mesh_.time().timeName(),  // Writing to the current time step directory
            directory,
            mesh_,  // Associated with the mesh
            IOobject::NO_READ,  // No need to read the file
            IOobject::NO_WRITE,  // Disable automatic writing
            false  // Don't register it as an object
        )
    );

    // Add data to the dictionary
    massDict.add("new_mass", mass);  // Add mass data to the dictionary
    massDict.add("density_ratio", density_ratio);  // Add mass data to the dictionary

    // Write the dictionary to the file
    bool writeSuccess = massDict.writeObject
    (
        IOstream::ASCII,          // ASCII format
        IOstream::currentVersion, // Use current OpenFOAM version
        IOstream::UNCOMPRESSED,   // Uncompressed output
        true                      // Write the file even if unchanged
    );

    // Optionally check if the file was successfully written
    if (!writeSuccess) {
        Foam::Info << "Error writing mass file!" << Foam::endl;
    } else {
        Foam::Info << "Mass file successfully written!" << Foam::endl;
    }
  }
}
...
void Foam::BprimeBoundaryConditions::updateBprimeBC()
{
  ...
  //Info << endl << "OUTPUT updateBprimeBC()" << endl;

  //Info << "updateBprimeBC()" << endl;

  volScalarField& rhoeUeCH=scalarFields_["rhoeUeCH"];
  //forAll(rhoeUeCH.boundaryFieldRef()[currentPatchID_], faceI) {
  //  Info << "\nfaceI: " << faceI << endl;
  //  const scalar& rhoeUeCH_BF = rhoeUeCH.boundaryField()[currentPatchID_][faceI];
  //  Info << "check prime 1" << endl;
  //  Info << "rhoeUeCH_BF:" << rhoeUeCH_BF << endl;
  //}  
  //raise(SIGTERM);
  Info << "\n\n HELLO 2 \n\n" << endl;

  int ne_Bprime = 3; //N O X

  csolid = new double[ne_Bprime];
  cwall = new double[ne_Bprime];
  cedge = new double[ne_Bprime];

  BprimeElement = new double[ne_Bprime];
  ...
  volScalarField& blowingCorrection=scalarFields_["blowingCorrection"];
  volScalarField& h_w=scalarFields_["h_w"];
  volScalarField& ce_0=scalarFields_["ce(0)"];
  volScalarField& ce_1=scalarFields_["ce(1)"];
  volScalarField& ce_2=scalarFields_["ce(2)"];
  volScalarField& cwall_0=scalarFields_["cwall(0)"];
  volScalarField& cwall_1=scalarFields_["cwall(1)"];
  volScalarField& cwall_2=scalarFields_["cwall(2)"];
  ...
  forAll(BprimeCw_.boundaryField()[currentPatchID_], faceI) {
    ...
    cedge[0] = ce_0.boundaryField()[currentPatchID_][faceI];
    cedge[1] = ce_1.boundaryField()[currentPatchID_][faceI];
    cedge[2] = ce_2.boundaryField()[currentPatchID_][faceI];

    cwall[0] = cwall_0.boundaryField()[currentPatchID_][faceI];
    cwall[1] = cwall_1.boundaryField()[currentPatchID_][faceI];
    cwall[2] = cwall_2.boundaryField()[currentPatchID_][faceI];
    ...
    Info << "     \ninside Bprime" << endl;
    Info << "ce_0_BF: " << cedge[0] << endl;
    Info << "ce_1_BF: " << cedge[1] << endl;
    Info << "ce_2_BF: " << cedge[2] << endl;
    Info << "cwall_0_BF: " << cwall[0] << endl;
    Info << "cwall_1_BF: " << cwall[1] << endl;
    Info << "cwall_2_BF: " << cwall[2] << endl;
    ...
    //Info << "     BprimeGw_BF:" << BprimeGw_BF << endl;
    //Info << "     BprimeCw_BF:" << BprimeCw_BF << endl;
    //Info << "     mDotCw_BF:" << mDotCw_BF << endl;
    //Info << "     h_w_BF:" << h_w_BF << endl;

//    forAll(p_Yke_, elementI) {
//      p_Yke_[elementI] = Yke_ref[elementI];
//      Info << "     p_Yke_[" << elementI << "]:" << p_Yke_[elementI] << endl;
//    }
...
//      Info << "     p_Ykg_[" << elementI << "]:" << p_Ykg_[elementI] << endl;
//    }
   ...
   // raise(SIGTERM);
    ...
    //  //Info <<  Xw_names[speciesI] << "]:" << p_Xw_[speciesI] << endl;
    //}
    //Info << endl << "HERE 2" << endl;

    BprimeCw_BF = 0.0;
    for(int i=0; i<ne_Bprime; i++) {
      Info << "     for " << i  << endl;
      Info << "     cwall[i] " << cwall[i]  << endl;
      Info << "     cedge[i] " << cedge[i]  << endl;
      Info << "     csolid[i] " << csolid[i]  << endl;
      
      BprimeElement[i] = 0.0;
      Info << "     BprimeElement[i] " << BprimeElement[i]  << endl;
      if (abs(csolid[i] - cwall[i]) > 1e-5) 
        {BprimeElement[i] = (cwall[i] - cedge[i])/(csolid[i] - cwall[i]);}
      Info << "     BprimeElement[i] " << BprimeElement[i]  << endl;
      BprimeCw_BF += BprimeElement[i];
    ...
    //BprimeCw_BF = mDotCw_BF / (rhoeUeCH_BF * blowingCorrection_BF);
...
Foam::wordList Foam::BprimeBoundaryConditions::neededFields()
{
  ...
  //neededFields.append("cwall(0)");
  //neededFields.append("cwall(1)");
  //neededFields.append("cwall(2)");
  return neededFields;
}
...
void Foam::BprimeBoundaryConditions::computeSurfaceMassBalance(scalarList Yke_, scalarList Ykg_, scalar pw_, scalar Tw_, scalar Bg_, scalar& Bc_, scalar& hw_, scalarList& p_Xw_)
{
  ...
  Info << endl << "OUTPUT computeSurfaceMassBalance()" << endl;
  ...
  if(readBprimeTable_) {
    ...
    for(int i = 0; i < ne_Bprime; i++) {
      Info << "     Yke[i]:" << Yke[i] << endl;
    }
    for(int i = 0; i < ne_Bprime; i++) {
      Info << "     Ykg[i]:" << Ykg[i] << endl;
    }
    Info << "     Tw:" << Tw << endl;
    Info << "     pw:" << pw << endl;
    Info << "     Bg:" << Bg << endl;
    ...
    Info << "FINISH OUTPUT MUTATION++:" << endl;
  ...
  Info << endl << "FINISH OUTPUT computeSurfaceMassBalance()" << endl;
}
