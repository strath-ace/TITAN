...
def adapt_mesh(amg,options,j, num_obj,input_grid, output_grid):
    ...
    subprocess.run(['python',path+'/Executables/su2io/su2gmf/su2_to_gmf.py', '-m',options.output_folder+'/CFD_Grid/'+ input_grid+'.su2', '-s', options.output_folder+'/CFD_sol/restart_flow_'+str(j)+'.csv','-o',options.output_folder+'/CFD_Grid/Amg/amg_'+str(j)])
    subprocess.run(['python',path+'/Executables/su2io/su2gmf/su2_to_gmf.py', '-m',options.output_folder+'/CFD_Grid/'+ input_grid+'.su2', '-s', options.output_folder+'/CFD_sol/restart_flow_'+str(j)+'.csv','-f',sensor,'-o', options.output_folder+'/CFD_Grid/Amg/sensor_'+str(j)])
    ...
    subprocess.run(['mv',options.output_folder+'/CFD_Grid/'+output_grid+'.csv',options.output_folder+'/CFD_sol/restart_flow_'+str(j+1)+'.csv'])
    subprocess.run(['cp',options.output_folder+'/CFD_sol/restart_flow_'+str(j+1)+'.csv',options.output_folder+'/CFD_sol/restart_flow.csv'])
